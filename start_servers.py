    ...
    if sys.platform == "win32":
        try:
            ...
            if "SUCCESS" in python_result.stdout:
                ...
                print(
                    f"{Colors.GREEN}Terminated existing Python processes.{Colors.ENDC}"
                )
            ...
            if "SUCCESS" in node_result.stdout:
                ...
                print(
                    f"{Colors.GREEN}Terminated existing Node.js processes.{Colors.ENDC}"
                )
        ...
        try:
            ...
            subprocess.run(
                ["pkill", "-f", "node"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )

            print(
                f"{Colors.GREEN}Attempted to terminate existing processes on Unix.{Colors.ENDC}"
            )
        except Exception as e:
            print(
                f"{Colors.RED}Error killing existing processes on Unix: {e}{Colors.ENDC}"
            )

    if killed_processes > 0:
        print(
            f"{Colors.GREEN}Successfully terminated {killed_processes} existing processes.{Colors.ENDC}"
        )
        # Give some time for processes to fully terminate
        time.sleep(2)
    else:
        print(
            f"{Colors.BLUE}No existing server processes found or they were already terminated.{Colors.ENDC}"
        )
...
def start_backend():
    ...
    try:
        ...
        print(
            f"{Colors.GREEN}Backend server started with PID: {backend_process.pid}{Colors.ENDC}"
        )
...
def start_frontend():
    ...
    if os.path.exists(os.path.join(FRONTEND_DIR, "yarn.lock")) and yarn_path:
        ...
        print(
            f"{Colors.RED}Error: Neither npm nor yarn was found in your PATH.{Colors.ENDC}"
        )
        print(
            f"{Colors.YELLOW}Please make sure Node.js is installed and in your PATH.{Colors.ENDC}"
        )
        return None
    ...
    try:
        ...
        print(
            f"{Colors.GREEN}Frontend server started with PID: {frontend_process.pid}{Colors.ENDC}"
        )
        ...
        try:
            ...
            print(
                f"{Colors.RED}Error starting frontend server (alternative method): {e2}{Colors.ENDC}"
            )
            return None
...
def main():
    ...
    def signal_handler(sig, frame):
        print(
            f"{Colors.YELLOW}Received keyboard interrupt. Shutting down...{Colors.ENDC}"
        )
        sys.exit(0)
    ...
    try:
        ...
        if frontend_process is None:
            print(
                f"{Colors.RED}Failed to start frontend server. Only backend is running.{Colors.ENDC}"
            )
            print(f"{Colors.BOLD}Backend URL: http://127.0.0.1:5000{Colors.ENDC}")
            print(
                f"{Colors.YELLOW}You can start the frontend manually with 'npm run dev' in another terminal.{Colors.ENDC}"
            )
            print(
                f"{Colors.YELLOW}Press Ctrl+C to stop the backend server{Colors.ENDC}"
            )

            # Just monitor the backend
            backend_thread = threading.Thread(
                target=monitor_output, args=(backend_process, "Backend")
            )
            ...
            print(
                f"{Colors.BOLD}{Colors.GREEN}Both servers are now running!{Colors.ENDC}"
            )
            print(f"{Colors.BOLD}Backend URL: http://127.0.0.1:5000{Colors.ENDC}")
            ...
            frontend_thread = threading.Thread(
                target=monitor_output, args=(frontend_process, "Frontend")
            )
            ...
            while True:
                ...
                if backend_process.poll() is not None:
                    print(
                        f"{Colors.RED}Backend server has stopped unexpectedly.{Colors.ENDC}"
                    )
                    break

                if frontend_process.poll() is not None:
                    print(
                        f"{Colors.RED}Frontend server has stopped unexpectedly.{Colors.ENDC}"
                    )
                    break
