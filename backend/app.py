...
def _archetype_id(name):
    """Deterministic ObjectId derived from the archetype name.

    In JSON-fallback mode each gunicorn worker seeds its own in-memory
    mongomock db. If archetypes are inserted without an _id, mongomock
    assigns a fresh random _id per worker, so the /api/archetypes list and a
    later /api/archetypes/<id>/cards request — which may be served by
    different workers — disagree on ids and cards intermittently come back
    empty. Deriving the _id from the (stable) name makes every worker agree.
    """
    return ObjectId(hashlib.md5(name.encode("utf-8")).hexdigest()[:24])


def _load_archetype_docs():
    return [{**a, "_id": _archetype_id(a["name"])} for a in ARCHETYPES]
...
def get_cards_internal(search, body_search, colors, color_match, exclude_colorless,
    ...
    if card_set and historic_mode:
        ...
        # For Set 5, we show all cards from Set 1, Set 2, Set 3, Set 4, and Set 5, plus cards with historical versions in any of these sets
        ...
        if card_set == "Set 1":
            ...
            sets_to_include = ["Set 1", "Set 2", "Set 3", "Set 4", "Set 5"]
        ...
        if sets_to_include:
            ...
            if card_set != "Set 5":  # Don't need to do this for Set 5 as it already includes everything
...
def get_archetype_cards(archetype_id):
    ...
    try:
        ...
        # Resolve the archetype by ObjectId, falling back to a string "id" or
        # the name. This keeps the lookup working even if the id doesn't parse
        # as an ObjectId or was generated by a different worker.
        archetype = None
        try:
            archetype = db.archetypes.find_one({"_id": ObjectId(archetype_id)})
        except Exception:
            archetype = None
        if not archetype:
            archetype = db.archetypes.find_one(
                {"$or": [{"id": archetype_id}, {"name": archetype_id}]}
            )
...
def get_chatgpt_response():
    ...
    try:
        ...
        logging.error(f"OpenAI API HTTP error: {http_err} - Response: {http_err.response.text}")
        return jsonify(
            {
                "error": f"OpenAI API error: {http_err.response.status_code}",
