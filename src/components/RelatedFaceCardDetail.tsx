...
export default function RelatedFaceCardDetail({ card, relatedCard, className = '' }: RelatedFaceCardDetailProps) {
  ...
  return (
    ...
    >
      <div className={`relative ${isRotatedCardType(card.type) ? 'battle-frame' : ''}`}>
        <img
          ref={imgRef}
          src={card.imageUrl || ''}
          alt={card.name || 'MTG Card'}
          className={isRotatedCardType(card.type) ? 'battle-rotate' : 'w-full h-full object-cover rounded-lg shadow-lg'}
          onError={(e) => {
